...
class OtherServicesAdScreen extends StatefulWidget {
  final Function(Locale) onLanguageChange;

  const OtherServicesAdScreen({Key? key, required this.onLanguageChange}) : super(key: key);

  @override
  State<OtherServicesAdScreen> createState() => _OtherServicesAdScreenState();
}

class _OtherServicesAdScreenState extends State<OtherServicesAdScreen> {
          ...
          child: Column(
            ...
            children: [
              ...
              _buildFormRow([
                ...
                _buildSingleSelectField(context, s.district, selectedDistrict, ['Jumeira', 'Al Barsha'], (selection) => setState(() => selectedDistrict = selection)),
              ]),
              const SizedBox(height: 7),
              
              _buildFormRow([
                 _buildTitledTextFormField(s.area, 'Jumeira1', _areaController, borderColor, currentLocale),
                 _buildTitledTextFormField(s.price, '1000', _priceController, borderColor, currentLocale, isNumber: true),
              ]),
              const SizedBox(height: 7),

               _buildFormRow([
                 _buildTitledTextFormField(s.serviceName, 'Account Audit', _serviceNameController, borderColor, currentLocale),
                 _buildSingleSelectField(context, s.sectionType, selectedSectionType, ['Financial Service', 'Legal Service'], (selection) => setState(() => selectedSectionType = selection)),
              ]),
              const SizedBox(height: 7),
              
              _buildTitleBox(context, s.title, 'Doing Account Editing Monthly', borderColor, currentLocale),
              const SizedBox(height: 7),
              
              TitledSelectOrAddField(
                title: s.advertiserName, 
                value: selectedAdvertiserName,
                items: ['Al Rasema Accounting Office', 'Prime Legal'],
              ...
              TitledDescriptionBox(title: s.description, initialValue: 'Specialized Accounting And Tax Audits Statements And Closing Monthly Accounts', borderColor: borderColor),
              ...
              Directionality(
                 ...
                 child: Row(
                  children: [
                    ...
                    SizedBox(width: 8.w),
                    Expanded(
                      child: Text('Dubai Jumeria', style: TextStyle(fontSize: 14.sp, color: KTextColor, fontWeight: FontWeight.w500)),
                    ),
                  ],
                ),
              ),
